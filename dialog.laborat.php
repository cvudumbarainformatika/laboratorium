...
<?php
$groppemeriksaan="";
$sql=$conn->query("select rs21,rs23 from rs49 group by rs21");
...
<table width="100%" cellpadding="0" cellspacing="0" border="1" bordercolor="#006699" bordercolordark="#666666" bordercolorlight="#003399">
	...
	<?php
	$z=0;
	$sql=$conn->query("select rs21,rs23,rs3,rs4,rs5,rs6,count(*) as jml from rs49 group by rs21");
	while($rs=$sql->fetch_object()){
	$z=$z+1;
	?>
		<?php if($rs->rs21==""){}else{?>
		...
		<?php
		$x=0;
		$sqlx=$conn->query("select * from rs49 where rs21='".$rs->rs21."' order by rs2");
		...
		<tr class="bodylist" bgcolor="<?php echo warna($x);?>">
			<td>&nbsp;&nbsp;</td>
			<td><input type="checkbox" name="cek<?php echo $rs->rs23;?>" value="<?php echo $rsx->rs1;?>" style="font-size:7px" onclick="PeriksaIsikan();" /></td>
...
function PeriksaIsikan(){
	...
	document.frmlaborat.kode_laborat.value=x;
